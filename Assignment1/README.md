...
Hadoop has a distributed file system, so it works fast with large amount of data with high scalability. On the other hand, it is not efficient with small size files that many small files produce lots of blocks and map task is inefficient because it processes a block of dat at a time.
